...
func _physics_process(delta: float):
	if !dead:
		match state:
			CHASE:
				move_anim.play("ghost_move")
			IDLE:
				move_anim.play("ghost_move_stop")
	else:
...
func _on_Hurtbox_area_entered(area: Area2D):
	if area.is_in_group("Hitbox") or area.is_in_group("Projectile"):
		if !hit and !dead:
			...
			create_popup_damage(area.damage, Color.white, Vector2(0.35, 0.35))
			hit_anim.play("ghost_hit")
